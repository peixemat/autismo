...
double gini_index(DataPoint* left, int n_left, DataPoint* right, int n_right) {
    int count[2];
    double gini = 0.0;
    int total = n_left + n_right;

    for (int side = 0; side < 2; ++side) {
        DataPoint* group = side == 0 ? left : right;
...
DecisionStump train_stump(DataPoint* data, int n_samples, int n_features) {
    DecisionStump best;
    ...
    for (int f = 0; f < n_features; ++f) {
        for (int i = 0; i < n_samples; ++i) {
            ...
            DataPoint left[MAX_SAMPLES], right[MAX_SAMPLES];
...
void bootstrap(DataPoint* data, int n_samples, DataPoint* out_sample) {
    for (int i = 0; i < n_samples; ++i) {
...
void train_forest(RandomForest* forest, DataPoint* data, int n_samples, int n_features, int n_trees) {
    forest->n_trees = n_trees;
    for (int i = 0; i < n_trees; ++i) {
        DataPoint bootstrap_sample[MAX_SAMPLES];
        bootstrap(data, n_samples, bootstrap_sample);
        forest->trees[i] = train_stump(bootstrap_sample, n_samples, n_features);
    }
}

int predict_tree(DecisionStump* tree, DataPoint* sample) {
    return sample->features[tree->feature_index] < tree->threshold ? tree->left_label : tree->right_label;
}

int predict_forest(RandomForest* forest, DataPoint* sample) {
    int votes[2] = {0};
...
void evaluate_model(RandomForest* forest, DataPoint* data, int n_samples, int* TP, int* FP, int* TN, int* FN) {
    *TP = *FP = *TN = *FN = 0;
    for (int i = 0; i < n_samples; ++i) {
